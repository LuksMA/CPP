...
vector<vector<double>> DataGeneration :: sampleGenerator(vector<int> const &seed, vector<double> const &ranges){
...
vector<vector<int>> DataGeneration :: sampleGenerator(vector<int> const &seed, vector<int> const &ranges){
    vector<vector<int>> vectOut;
    int noVariable = getRangeSize(ranges);
    int noSample = getSampleSize();
    for(unsigned int i=0 ; i < noVariable; ++i){
...
double DataGeneration :: getSumT1(){
    return sumTreatment1;
}
