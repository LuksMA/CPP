#include <iostream>
#include <omp.h>

#include "DataGeneration.h"
#include "ITR.h"


using namespace std;

int main()
{
    /// Test data generation class
    vector<int> varType(2,0); /// 0 for cont, 1 for ordinal, 2 for nominal
    vector<int> varType1(1,1);
    vector<int> varType2(2,2);
    varType.insert(varType.end(),varType1.begin(),varType1.end());
    varType.insert(varType.end(),varType2.begin(),varType2.end());

    vector<double> rangesCont = {50.0,300.0,0.0,100.0};
    vector<int> rangesOrd = {0,4};
    vector<int> rangesNom = {0,4,0,4};
    vector<double> rangesY = {0.0,100.0};
    vector<int> rangesAction = {0,1};

    DataGeneration data(varType,rangesY,rangesAction,rangesCont,rangesOrd,rangesNom);
    data.creatSamples(30);
    ITR ITR_CS(data,3);
//    ITR_CS.print_VarInfo();
//    ITR_CS.print_CutTable();
//    ITR_CS.print_Combinations();
//    ITR_CS.print_Range();
    ITR_CS.DepthPrint();
//    ITR_CS.threeDepthPrint();
//    ITR_CS.threeDepth();

}



